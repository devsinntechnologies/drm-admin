...
function TablesContent() {
  ...
  const onOpenEdit = async (id: string) => {
    ...
    try {
      ...
      setEditId(id);
      setExistingImage(table.image || null);
      setForm({
      ...
      <form className="p-8 space-y-6" onSubmit={onSubmit}>
        <div className="space-y-2">
          ...
          <input
            ...
            className="w-full h-14 bg-slate-100/50 rounded-2xl px-6 text-sm font-bold border-none outline-none focus:ring-2 ring-[#ef4444]/20 transition-all placeholder:text-gray-400"
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            <input
              ...
              min={1}
              value={form.capacity}
              onChange={(e) => setForm((p: any) => ({ ...p, capacity: Math.max(1, Number(e.target.value)) }))}
              className="w-full h-14 bg-slate-100/50 rounded-2xl pl-6 pr-12 text-sm font-bold border-none outline-none focus:ring-2 ring-[#ef4444]/20 transition-all [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
              required
            />
            <div className="absolute right-3 top-1/2 -translate-y-1/2 flex flex-col items-center">
              <button
                type="button"
                onClick={() => setForm((p: any) => ({ ...p, capacity: p.capacity + 1 }))}
                className="p-1 text-slate-400 hover:text-[#111827] transition-colors"
              >
                <ChevronUp className="h-3 w-3" strokeWidth={4} />
              </button>
              <button
                type="button"
                onClick={() => setForm((p: any) => ({ ...p, capacity: Math.max(1, p.capacity - 1) }))}
                className="p-1 text-slate-400 hover:text-[#111827] transition-colors"
              >
          ...
          <div className="flex items-start gap-6">
            ...
            {displayImage && (
              <div className="flex flex-col gap-2">
                <div className="relative h-24 w-36 rounded-2xl overflow-hidden border border-slate-200 bg-slate-50 shadow-sm">
                  <img
                    src={displayImage}
                    alt="Preview"
                ...
                {form.image && (
                  <span className="text-[10px] font-bold text-slate-500 truncate max-w-[144px]">
                    {form.image.name}
                  </span>
                )}
              </div>
