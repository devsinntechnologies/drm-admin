...
const WEBSITE_BASE = "/dashboard/businessAdmin/website";

const tabs = [
  { key: "overview", href: WEBSITE_BASE, label: "Overview", icon: Globe2 },
  { key: "pages", href: `${WEBSITE_BASE}/pages`, label: "Pages", icon: FileText },
  { key: "theme", href: `${WEBSITE_BASE}/theme`, label: "Theme", icon: LayoutTemplate },
  { key: "domain", href: `${WEBSITE_BASE}/domain`, label: "Domain", icon: Link2 },
] as const;

function isTabActive(pathname: string, tabHref: string, tabKey: string) {
  const path = tabHref.split("?")[0];
  if (tabKey === "overview") {
    return pathname === WEBSITE_BASE;
  }
        ...
        <div className="flex min-w-0 items-center gap-4">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[0.92rem] text-[#6c7890] sm:text-[0.98rem]">
              Create, publish, and connect a customer website built with DigiNizam
            </p>
          </div>
        </div>
      </section>
