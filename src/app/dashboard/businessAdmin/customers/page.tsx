...
function CustomersContent() {
  ...
  return (
    <PharmacyPage moduleId="customers" icon={Users} title="Patients & loyalty" subtitle="Medication profiles, allergies, and loyalty points" loading={loading} error={error}>
      ...
      <form className="mt-4 grid gap-3 rounded-2xl border border-[#e2e8f0] bg-white p-4 md:grid-cols-5" onSubmit={(e) => {
        ...
        <input className={portalInputClass} placeholder="Patient ID" value={form.patientIdNumber} onChange={(e) => setForm({ ...form, patientIdNumber: e.target.value })} />
      ...
      <DataTable columns={[{ key: "name", label: "Name" }, { key: "phone", label: "Phone" }, { key: "id", label: "Patient ID" }, { key: "allergies", label: "Allergies" }, { key: "points", label: "Points" }]} rows={rows} empty={<PortalEmptyState icon={Users} title="No patients yet" />} />
    </PharmacyPage>
  );
}
